...
=== b
Dans le cas d'une recherche fructueuse, soit $k$ la profondeur du nœud. On aura une comparaison par échec (tant que l'on est pas encore au nœud) et une comparaison pour valider que la clé est bien la bonne. On aura donc bien $k$ comparaisons, c'est à dire $k$ nœuds parcourus. Dans le cas d'une recherche infructueuse, on note $k_p$ et $k_s$ la profondeur de son prédecesseur et successeur, au sens des clés, respectivement. Un nœud absent serait toujours contenu entre son prédecesseur et son succésseur. De la structure d'un arbre cartésien, si il n'y a aucun nœud entre un prédecesseur et son succésseur, c'est à dire que le nœud recherché n'est pas présent, alors on a deux scénario possible. Soit le prédecesseur est le père du succésseur, soit le succésseur est le père du prédecesseur. Pour trouver une feuille vide, il faut donc aller jusqu'à $max{k_p, k_s}$. 
...
=== c
_Voir la fonction `CartesianTree::delete` pour la suppression d'un nœud dans l'arbre._

=== d
_Voir la fonction `exercice_4` pour un le test de suppression de nœuds dans l'arbre._

== Exercice 5

=== a
L'implémentation de la mesure de performance a été réalisé dans les fichiers `include/performance.h` et `src/performance.cpp`.
...
La méthodologie de test pour mesurer et comparer est la suivante. Tout d'abord, nous avons testé les métriques de temps d'exécution sur trois structures de données différentes : l'arbre cartésien, mais aussi un arbre binaire de recherche et un tas binaire. Ces deux structures ont été implémentée dans `include/BinarySearchTree.hpp` et `include/BinaryHeap.hpp` et ont été choisies parce que les arbres cartésients combinent les propriétées de ces deux arbres. Pour les deux arbres, on a testé les métriques structurelles (ces métriques n'ont pas de sens pour les tas binaires).

On a aussi voulu tester plusieurs variations d'insertion et de collisions. Pour le tas, on a fait que tester pour chaque taille d'instance. Pour les arbres, on a généré des données aléatoires initiale, mais on a créé les arbres avec trois stratégies distinctes, soit au hasard, soit par ordre croissant au sens des clés, soit par ordre décroissant au sens des clés. Finalement, pour l'arbre cartésien, on a aussi mesuré l'impact des collisions sur la performance, on a donc introduit $alpha in [0, 1]$ qui correspond à la réduction du domaine de choix pour les clés ($alpha = 0.1$ veut dire que au lieu d'avoir $n$ clés distinctes, on aura $n/100$ clés distinctes).

On a évaluer à différentes tailles d'instance pour les différentes structures de données, et on a effectué chaque test en faisant 50 tirages aléatoires pour la création des données de la structure et en notant sa médiane.
...
On observe la performance des trois opérations fondamentales sur un arbre binaire est parmis tous nos graphes de loin la pire. Ceci est due à un cas particulier : si on trie les nœuds à ajouter par ordre de clé décroissant, alors non seulement l'arbre est désequilibré (ce qui est aussi le cas sur un ordre croissant), mais on se retrouve dans le pire cas pour l'insertion et la supression. On peut voir sur ces graphes exactement à quelle point ce désequilibre a un impact sur les performances réelles de nos graphes.

On note aussi que nous avons choisi de ne pas représenter la vitesse de recherche dans toutes nos structures parce que nos mesures étaient toujours 0. Sur cette taille d'arbre, le fait de ne pas avoir a alouer de mémoire et de faire au pire un simple parcous suffit a rendre la vitesse de cette opérations complètement négligeable.
...
Sur la performance de nos arbres cartésiens, on observe plusieurs résultats clés. Le plus clair étant la dégradation de performance dans le cas de priorité aléatoire. La dégradation en elle même n'est pas étonnante, car toutes les opérations sont en $O(k)$ avec $k$ la profondeur moyenne d'un nœud, qui comme on le démontre dans l'exercice 6 est en $O(log n)$. Cela veut dire que la performance des opérations devrait être en $O(log n)$. On observe bien une courbe logarithmique, lorsque l'on suit une stratégie d'insertion aléatoire. Cela étant dit, il est donc plus surprenant d'observer que les stratégies d'insertion croissante et décroissante résulte en un gain de performance. On peut supposer que ce gain de performance est du à une simplification dans la construction de l'arbre, qui résulte en moins de rotation à faire, et que la courbe est aussi exponentielle, même si cela est moins évident à voir.

On peut aussi voir sur ces graphes l'impact du paramètre $alpha$. On constate qu'il induit une dégradation, qui est en particulier plus notable lorsque l'on suit une stratégie décroissante. Cette observation pratique sera justifié dans le reste de la réflexion.
...
Sur les arbres cartésiens, on a plus de donnée. Commençon par la hauteur et la profondeur moyenne. On peut observer, comme on va le calculer dans l'exercice 6, qu'on suit bien une courbe de type $O(log n)$. Cela est un point déterminant puisque cela permet d'avoir une complexité $O(log n)$ sur le reste de nos opérations fondamentales. Finalement, on pourrait observer que le paramètre $alpha$ joue sur la profondeur moyenne de l'arbre, avec les valeurs élevée menant à des arbres plus profond, mais ce n'est pas un résultat que nous avons pu observer empiriquement de façon claire.
...
On remarque que notre structure d'arbre binaire de recherche peut être extrèmement déséquilibré. En effet, dans la stratégie d'insertion par ordre croissant et par ordre décroissant, on note des arbres qui ne sont pas du tout équilibrées. Ceci est particulièrement problématique car cela rend des arbres binaires de recherche sembable à des listes chainées, qui sont connues pour n'être en pratique pas très performante. Ceci explique la performance extrèmement mauvaise de nos arbres binaires de recherche lorsque l'on suit ces stratégies.

Cela laisse la question de ce pourquoi les arbres cartésiens ne souffrent pas de ce problème. Le fait d'avoir une priorité aléatoire force des rotations. En effet, lorsqu'il y a une rotation, la rotation force un rééquilibrage sur le sous-arbre où la rotation a lieu. Comme les prioritées sont aléatoires, alors le point de rééquilibrage est completement choisi au hasard. Sur des grandes instances de l'arbres, on observe que ce processus atteint une moyenne et permet un rééquilibrage en moyenne de l'arbre. Finalement, comme les priorités sont choisies au hasard, alors on peut pas trouver de stratégies qui pourraient déséquilibrées ces arbres.

On a introduit le paramètre $alpha$ pour pouvoir observer précisement l'impact des collisions sur la performances dans ce type de stratégies de construction. On observe que lorsque $alpha$ monte, cela mène à plus de collision et empiriquement, que les performances se degradent. En effet, on se ramène petit à petit à un simple arbre binaire de recherche et on s'attend à la même dégradation des performance. On peut même envisager un cas théorique, où $alpha = 1$, c'est à dire que toutes les priorités sont identiques. Dans ce cas, au moment de l'insertion, la proriété de tas est toujours respectée, donc on a jamais de rotation, donc on a exactement un arbre binaire de recherche, avec les même problèmes.
...
($<==$) On sait que $x_i$ est le nœud qui a la plus petite priorité dans $X(i,k)$. Alors par construction de l'arbre cartésien, par prioriété du tas, il en sera la racine. Il sera donc l'ancêtre de tous les nœud du sous arbre, en particulier de $x_k$. On aura donc bien, par définition, $X_(i k) = 1$.
